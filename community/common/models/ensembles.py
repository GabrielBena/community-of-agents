...
class Community(nn.Module) : 
    ...
    def nb_connections(self) : 
        ...
        """
        if self. use_deepR : 
            return {tag : (c.thetas[0]>0).int().sum() for tag, c in zip(self.connections.keys(), self.connections.values())}
        else : 
            return {tag : (c.w_mask>0).int().sum() for tag, c in zip(self.connections.keys(), self.connections.values())}
...
class ConvCommunity(nn.Module) : 
    ...
    def init_connections(self, sparse_connections=None) : 
        ...
        if sparse_connections is None : sparse_connections = self.sparse_connections
    ...
    def forward(self, x, supsup=False, temporal_masks=False):
        ...
        for n, x_t in enumerate(x) : 
                ...
                if n > 0 : 
                    ...
                    if not supsup : 
                        for j, ag2 in enumerate(self.agents) :
                            if self.connected[j, i]==1: 
                                #State-to-state connections j->i
                                sparse_out = self.connections[self.tags[j, i]](states[n-1][j])
            ...
            if self.use_attention : 
                ...
                output, scores = self.attention_layer(output, state_decider.transpose(0, 1), state_decider.transpose(0, 1))
                output = output.transpose(0, 1)
                #return scores
