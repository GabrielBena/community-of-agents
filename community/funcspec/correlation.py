...
try:
    ...
    skip_cka = True
    print("CKA not installed, cannot compute CKA metric")
    pass
...
def get_correlation(community, data, method="pearson"):
    ...
    if method == "pearson":
        ...
        cor = np.stack([v_cka(ag_s, ag_s[perm])[0] for ag_s in agent_states])
    else:
        raise NotImplementedError('Method not recognized ("pearson" or "cka")')
...
def get_pearson_metrics(
    ...
    if method == "pearson":
        similarity_f = lambda s1, s2: (v_pearsonr(s1, s2)[0].mean() + 1) / 2
    elif method == "cka":
        try:
            cka = CKA()
            similarity_f = lambda s1, s2: cka.linear_CKA(s1, s2)
        except NameError:
            print("CKA not installed, cannot compute CKA metric")
            return
    else:
    ...
    for _ in pbar_0:
        for (datas, label), _ in zip(pbar_1, range(100)):
            ...
            base_corrs = np.array(
                [
                    [similarity_f(s, s[perm(s)]) for s in s_ag]
            ...
            for target_digit in [0, 1]:
                ...
                states_0 = [
                    s[:, 0].cpu().data.numpy() for s in states if 0 not in s.shape
                ]
                states_1 = [
                    s[:, 1].cpu().data.numpy() for s in states if 0 not in s.shape
                ]

                corrs_0 = np.array(
                    [
                        [
                            similarity_f(s, s[perm(s)])
    ...
    try:
        correlations = np.stack(
            correlations, -2
        )  # n_batches x n_timesteps x n_agents x n_targets x n_classes
        ...
        # print(mean_corrs.shape, relative_corrs.shape, base_corrs.shape)
...
def process_correlations(correlations, base_correlations):
    mean_corrs = correlations.mean(-1).mean(0)  # n_timesteps x n_agents x n_targets
    base_corrs = base_correlations.mean(-1)  # n_timesteps x n_agents
    base_corrs = np.stack(
        [base_corrs for _ in range(2)], -1
    )  # n_timesteps x n_agents x n_targets

    relative_corrs = np.clip(mean_corrs - base_corrs, 0, 1) / np.clip(
        1 - base_corrs, 0, 1
    )  # n_timesteps x n_agents x n_targets
