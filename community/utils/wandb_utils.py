...
def get_wandb_runs(
    run_id=None, config=None, entity="gbena", project="funcspec", process_config=False
):
...
def get_wandb_artifact(
    ...
    if len(runs) != 1:
        print(
            f"Warning : {len(runs)} runs found for current filters"
        )  # , taking last one by default as no run id is specified')
    artifacts = [r.logged_artifacts() for r in runs]

    wanted_artifacts = [
        [art for art in artifact if name in art.name] for artifact in artifacts
    ]
    wanted_artifacts = [art for art in wanted_artifacts if len(art) > 0]
    assert (
        len(wanted_artifacts) > 0
    ), f"No artifacts found for name {name} or type {type} for {len(runs)} currently filtered runs"

    if len(wanted_artifacts) != 1:
        print(
            f"Warning : {len(wanted_artifacts)} runs containing wanted artifact, taking last one by default as no run id or precise name is specified"
        )
    wanted_artifact = wanted_artifacts[0]

    if len(wanted_artifact) != 1:
        print(
            f"Warning : {len(wanted_artifacts)} artifacts found for single run, taking last one by default"
        )
    wanted_artifact = wanted_artifact[0]

    if ensure_latest:
        assert (
            "latest" in wanted_artifact.aliases
        ), "Artifact found is not the latest, disable ensure_latest to return anyway"
