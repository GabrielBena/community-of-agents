...
def step_connections_spiking(model, optimizer_connections, global_rewire, thetas_list, sparsity_list, deepR_params_dict) : 
    """
    ...
    for tag, connect in zip(model.connections.keys(), model.connections.values()) : 
        if type(connect) is LIFLayerConnect : 
            connect = connect.base_layer
            if type(connect) is deepR.Sparse_Connect :
                for theta in connect.thetas : 
                    apply_grad(theta, deepR_params_dict)

    optimizer_connections.step()

    #Rewire sparse connections
    nb_new_con = torch.tensor(0)
                ...
                if type(connect) is deepR.Sparse_Connect :
                    for theta, nb_connect in zip(connect.thetas, connect.nb_non_zero_list) : 
                        if theta.requires_grad : 
                            ''
                            nb_new_con += deepR.rewiring(theta, nb_connect)
                        else : 
                            nb_new_con += 0

    else : 
        nb_reconnections = deepR.sample_matrix_specific_reconnection_number_for_global_fixed_connectivity(thetas_list, sparsity_list)
        for connect, nb_reconnect in zip(model.connections.values(), nb_reconnections) : 
            if type(connect) is LIFLayerConnect : 
                connect = connect.base_layer
                if type(connect) is deepR.Sparse_Connect :
                    if theta.requires_grad : 
                        nb_new_con += deepR.rewiring_global(connect.thetas[0], nb_reconnect)
                    else : 
                        nb_new_con += 0
