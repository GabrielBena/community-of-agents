...
if __name__ == "__main__": 
    ...
    if config['varying_param'] == 'sparsity' : 
        try : 
            varying_params = (np.geomspace(p_cons_params[0], p_cons_params[1], p_cons_params[2]) * agents_params_dict['n_hid']**2).round()
        except ValueError : 
            varying_params = (np.linspace(p_cons_params[0], p_cons_params[1], p_cons_params[2]) * agents_params_dict['n_hid']**2).round()

        varying_params = np.unique(varying_params / agents_params_dict['n_hid']**2)
        
    elif config['varying_param'] == 'n_hidden' : 
        varying_params = np.linspace(n_hids_params[0], n_hids_params[1], n_hids_params[2], dtype=int)
